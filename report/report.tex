...
Another observation that can be made is that the \textit{n\_conflicts} grows as the size of the graph increases and therefore of the memory required for the search space.
A high number of conflicts could indicate that the solver tried lots of assignments that did not satisfy the formula so we can conclude that Z3 manage to explore the seach space in the direction of goal better than CPLEX.
\begin{figure}[h!]
\begin{center}
